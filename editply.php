...
<body>
  ...
  <div>
    ...
    <input type="file" class="form-control"  name="pucture2"required>
  </div>
  <div class="mb-3">
  ...
  <button type="submit" name="valider" class="btn btn-primary">EDIT</button>
</form>
</div>
 <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
...
{
	if(isset($_POST['id2']) AND isset($_POST['pucture2']) AND isset($_POST['name2']) AND isset($_POST['price2']) AND isset($_POST['description2']))

{
	if(!empty($_POST['id2']) AND !empty($_POST['pucture2']) AND !empty($_POST['name2']) AND !empty($_POST['price2']) AND !empty($_POST['description2']))
     {
...
try {
	modifier($id,$pucture,$name,$price,$description);
